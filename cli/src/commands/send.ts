...
export async function send(to: Address, amount: string, privateKey: `0x${string}`, opts: { token?: Address; tokenId?: string }) {
  const pub = getPublicClient();
  const wallet = getWalletClient(privateKey);
  const account = wallet.account;

  if (opts.token && opts.tokenId) {
    const tokenId = BigInt(opts.tokenId), qty = BigInt(amount);
  ...
  if (opts.token) {
    ...
    console.log(`💸 Sending ${amount} ${symbol} (${shortAddr(opts.token)}) to ${shortAddr(to)} on Base...`);
    const hash = await wallet.writeContract({
      address: opts.token, abi: [{ type: 'function', name: 'transfer', stateMutability: 'nonpayable', inputs: [{ name: 'to', type: 'address' }, { name: 'amount', type: 'uint256' }], outputs: [{ type: 'bool' }] }] as const,
